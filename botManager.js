...
function deleteBot(serverId, serverKey, storedKey) {
  return new Promise((resolve, reject) => {
    ...
    try {
      // Ensure all intervals are cleared before ending the bot
      if (bot._actionIntervals) {
        bot._actionIntervals.forEach(interval => clearInterval(interval));
        bot._actionIntervals = null;
      }
      
      bot.end();
      delete bots[serverId];
      console.log(`Bot ${serverId} successfully deleted`);
      resolve();
    } catch (err) {
      console.error(`Error deleting bot ${serverId}:`, err.message);
      reject(err);
    }
  });
}
...
function setupBotActions(bot) {
  ...
  // Store intervals for cleanup later
  intervals.push(setInterval(rotateHead, 300000));
  intervals.push(setInterval(moveRandomly, 5000));
  intervals.push(setInterval(preventAfk, 60000));
  
  // Attach the intervals to the bot for cleanup when deleted
  bot._actionIntervals = intervals;
  
  // Add cleanup handler for end event
  bot.once('end', () => {
    if (bot._actionIntervals) {
      bot._actionIntervals.forEach(interval => clearInterval(interval));
      bot._actionIntervals = null;
